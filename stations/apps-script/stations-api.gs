 ...
 *
 * القراءة والكتابة كلتاهما خلف تسجيل الدخول والصلاحية (publicRead = false).
 * الشيت نفسه يبقى غير مشارَك: الواجهة تقرأ نيابة عن الزائر، فلا يصل أحد إلى
 * الملف الخام ولا إلى أوراق المستخدمين والجلسات.
 *
 * لفتح القراءة للزائر بلا حساب: اجعل CONFIG.publicRead = true.
 *
...
var CONFIG = {
  ...
  timeZone: 'Asia/Dubai',
  // مقفلة: لا يعود صف واحد قبل تسجيل الدخول — القراءة كالكتابة.
  // اجعلها true لتفتح القراءة للزائر بلا حساب، وتبقى الكتابة خلف الصلاحية.
  publicRead: false,
  sessionHours: 12,
  hashRounds: 4000,
  maxAttempts: 5,
