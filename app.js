...
(() => {
  ...
  function renderRows(){
    ...
    $('rows').innerHTML = visible.map(({item,index})=>{
      ...
      return `<tr class="record-row ${expanded.has(item.id)?'is-open':''}" id="row-${escape(item.id)}"><td class="number-cell"><span class="row-number">${index}</span></td><td class="topic-cell"><h4 class="item-title">${escape(item.title)}</h4><span class="reference" dir="auto">${escape(item.reference)}</span><div class="record-context"><span>${escape(F.homeLabels[F.home(item,data)])}</span>${F.sources(item,data).filter(source=>source!=='unknown').map(source=>`<span class="source-tag">${escape(F.sourceLabels[source])}</span>`).join('')}</div>${item.blocker?`<span class="blocker">${escape(item.blocker)}</span>`:''}</td><td class="status-cell"><div class="badges"><span class="priority priority-${escape(item.priority)}">${priorities[item.priority]}</span>${inWorkshop(item)?`<span class="gear-badge">${escape(window.WorkshopSheets.handovers[item.equipment.handover])}</span>`:''}</div><span class="stage stage-${escape(item.stage)}">${labels[item.stage]}</span></td><td class="action-cell"><p class="next-action">${escape(item.action)}</p><span class="owner">${escape(item.owner)}</span><span class="action-at" data-at="${escape(item.actionAt||'unassigned')}">${escape(actionLabels[item.actionAt]||actionLabels.unassigned)}</span></td><td class="date-cell"><time class="information-date" datetime="${escape(item.informationDate)}">${escape(shortDate(item.informationDate))}</time><span class="date-note ${isOld?'':'current'}">${isOld?'حالة من السجل الأساسي':'تحديث مسجل'}</span></td><td class="expand-cell">${window.WorkshopAdmin?.canEdit()?`<button type="button" class="edit-button" data-edit="${escape(item.id)}" aria-label="تعديل ${escape(item.title)}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h4L19 9a2.8 2.8 0 0 0-4-4L4 16Z"/></svg></button>`:''}<button type="button" class="expand-button" data-item="${escape(item.id)}" aria-label="تفاصيل ${escape(item.title)}" aria-expanded="${expanded.has(item.id)}" aria-controls="detail-${escape(item.id)}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg></button></td></tr>${details(item)}`;
    }).join('');
    status(`${visible.length}/${items.length} ${I.t('متابعة معروضة')}`);
  ...
  // رسالة واحدة مركّبة لقارئ الشاشة بدل جعل كل شبكة منطقة حيّة تتنافس.
  function status(message,scope='overview'){
    // اللوحة المعروضة وحدها تكتب، وإلا كتب آخرها فأعطى عدداً لا يطابق ما يراه المستخدم.
    const view=window.WorkshopViews?.current||'overview';
    const owner=view==='letters'?'letters':view==='jobs'?'jobs':'overview';
    if(scope!==owner)return;
    const node=$('results-status');
  ...
  window.WorkshopApp={detailContent,refresh:()=>fetchData(true),notice,status,reveal(id){
